...
,update: function(output, domElement) {
  ...
  for (var i = 0; i < data.fan['total-fans-in-system']; i++) {
    ...
    $('#stats .fan-' + i + ' .temp').text(Math.floor(data.fan['fan-' + i + '-speed']) + '');
  }
}
...
,renderChart: function(title, icon, percentage, temp) {
  ...
  var html  = '<div class="chart ' + title.replace(/\s/g, '-').toLowerCase() + '" ';
      html +=      'style="top:' + this.ui.top + '; left:' + this.ui.left + '; color:' + this.ui.labelcolor + '">';
...
,style: "                                                    \n\
  ...
  @font-face                                                 \n\
    font-family: 'Icons';                                    \n\
    src: url('SideBar.widget/istats.widget/icons.ttf') format('truetype')   \n\
    font-weight: normal;                                     \n\
    font-style: normal;                                      \n\
                                                             \n\
